...
type Resources struct {
	...
	//   - A Package represents a subdivision of the *total node/numa resources*
	//     into CPU:Memory ratio bundles (e.g. "x2" = 1:2, "x8" = 1:8).
...
//
// The naming convention is based on CPU:Memory ratio:
//   - Example: "x2" means 1 core : 2 GiB memory
//   - Example: "x8" means 1 core : 8 GiB memory
//
// Pods associated with this package will consume resources with
// the same CPU:Memory ratio.
// A special "default" resource package is implied: it represents all the
// resources not explicitly assigned to a named resource package.
type ResourcePackage struct {
	// PackageName is the identifier for this package, e.g. "x2", "x8".
	// Must either be:
	//   - "xN" (N = integer, e.g. x2, x8, x32)
	//   - "default" or "default-N" (N = integer)
	//
	// +kubebuilder:validation:Pattern=`^(x[0-9]+|default(-[0-9]+)?)$`
