...
// nodeProfileDescriptorLister implements the NodeProfileDescriptorLister interface.
type nodeProfileDescriptorLister struct {
...
func NewNodeProfileDescriptorLister(indexer cache.Indexer) NodeProfileDescriptorLister {
	return &nodeProfileDescriptorLister{indexer: indexer}
}

// List lists all NodeProfileDescriptors in the indexer.
func (s *nodeProfileDescriptorLister) List(selector labels.Selector) (ret []*v1alpha1.NodeProfileDescriptor, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.NodeProfileDescriptor))
	})
	return ret, err
}

// Get retrieves the NodeProfileDescriptor from the index for a given name.
func (s *nodeProfileDescriptorLister) Get(name string) (*v1alpha1.NodeProfileDescriptor, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("nodeprofiledescriptor"), name)
	}
	return obj.(*v1alpha1.NodeProfileDescriptor), nil
}
